...
//  Project26
//
//  Created by Alperen Çerkez on 24.12.2024.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func loadLevel() {
        ...
        for (row, line) in lines.reversed().enumerated() {
            for (column, letter) in line.enumerated() {
                ...
                createNode(for: letter, at: position)
            }
        }
    }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if let accelerometerData = motionManager?.accelerometerData {
            physicsWorld.gravity = CGVector(dx: accelerometerData.acceleration.y * -50,  accelerometerData.acceleration.x * 50)
        }
        #endif
    }
    ...
    func teleportPlayer() {
        let teleportNodes = children.filter { $0.name == "teleport" && $0 != player }
        guard let randomTeleport = teleportNodes.randomElement() else { return }
        
        let move = SKAction.move(to: randomTeleport.position, duration: 0.5)
        player.run(move)
    }
